...
namespace OxyPlotBenchmark
{
    public class Vm : INotifyPropertyChanged
    {
        ...
        {
            _xs = Enumerable.Range(1, NumberOfPoints).Select(x => 2 * Math.PI * x / NumberOfPoints).ToArray();
            double[] y1 = _xs.Select(x => Math.Sin(x)).ToArray();
            double[] y2 = _xs.Select(x => Math.Sin(x + Math.PI / 2)).ToArray();
            double[] y3 = _xs.Select(x => Math.Sin(x + Math.PI)).ToArray();
            double[] y4 = _xs.Select(x => Math.Sin(x + 1.5 * Math.PI)).ToArray();
